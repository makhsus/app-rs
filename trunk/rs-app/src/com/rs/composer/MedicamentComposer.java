package com.rs.composer;

import java.util.List;

import org.hibernate.criterion.Order;
import org.zkoss.zk.ui.select.annotation.Listen;
import org.zkoss.zk.ui.select.annotation.Wire;
import org.zkoss.zul.Listbox;
import org.zkoss.zul.Listcell;
import org.zkoss.zul.Listitem;
import org.zkoss.zul.Toolbarbutton;

import com.rs.dao.MedicamentDao;
import com.rs.model.Medicament;
import com.rs.util.CommonUtil;

public class MedicamentComposer extends BaseComposer {
	private static final long serialVersionUID = 1L;

	@Wire
	private Listbox lbxObat;
	
	
	@Listen ("onCreate = #win")
	public void win(){
		isLooged();
		loadDataObat();
	}
	
	private void loadDataObat(){
		MedicamentDao dao = new MedicamentDao();
		dao.setSessionFactory(sessionFactory);
		List<Medicament> list = dao.loadAll(Order.asc("medicamentName"));
		
		lbxObat.getItems().clear();
		int no = 1;
		for(Medicament obj: list){
			Listitem li = new Listitem();
			lbxObat.appendChild(li);
			
			Listcell lc = new Listcell(Integer.toString(no));
			li.appendChild(lc);
			lc = new Listcell(obj.getMedicamentName()!=null ? obj.getMedicamentName().toUpperCase():"");
			li.appendChild(lc);
			lc = new Listcell(obj.getMedicamentCode()!=null ? obj.getMedicamentCode():"");
			li.appendChild(lc);
			lc = new Listcell(obj.getSupplier()!=null ? obj.getSupplier().getSupplierName():"");
			li.appendChild(lc);
			lc = new Listcell(obj.getMedicamentUnit()!=null ? obj.getMedicamentUnit():"");
			li.appendChild(lc);
			lc = new Listcell(obj.getPriceBuy()!=null ? CommonUtil.formatStringIDR(obj.getPriceBuy().intValue()):"");
			li.appendChild(lc);
			lc = new Listcell(obj.getPriceSell()!=null ? CommonUtil.formatStringIDR(obj.getPriceSell().intValue()):"");
			li.appendChild(lc);
			
			
			lc = new Listcell();
			Toolbarbutton tbn = new Toolbarbutton("Edit");
			lc.appendChild(tbn);
			li.appendChild(lc);
			
			no++;
		}
		
	}
}
